...
import type { HeatmapDay, VolumeData, VolumeTrendData } from "../repositories/analytics.repository";
...
export interface AnalyticsService {
  ...
  getVolumeTrend(daysBack?: number): Promise<VolumeTrendData[]>;
...
export function createAnalyticsService(
  ...
  return {
    ...
    async getVolumeTrend(daysBack = 14): Promise<VolumeTrendData[]> {
      return analyticsRepo.getVolumeTrend(daysBack);
    },
